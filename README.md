# Autenix

O Autenix é um sistema digital criado para modernizar o atendimento em restaurantes, substituindo o cardápio físico tradicional por um QR Code exclusivo para cada mesa.

Ao chegar no restaurante, o cliente apenas escaneia o QR Code com a câmera do celular e automaticamente acessa o cardápio completo, sem precisar baixar nenhum aplicativo. A partir daí, ele pode navegar pelos produtos, ver descrições, preços e imagens, escolher o que deseja e fazer o pedido diretamente pelo próprio celular.

Depois de enviar o pedido, o cliente consegue acompanhar o andamento em tempo real, sabendo quando o pedido está sendo preparado ou quando já está pronto. Tudo isso torna o atendimento mais rápido, reduz erros de comunicação e melhora a experiência tanto para o cliente quanto para o restaurante.

---

## O que o sistema faz

* Permite acessar o cardápio pelo celular
* Faz pedidos direto pela mesa
* Mostra o status do pedido (preparando, pronto, etc.)
* Organiza pedidos por mesa
* Envia pedidos para a cozinha
* Possui um painel para o restaurante gerenciar tudo
* Possui controle financeiro dos pedidos

---

## Tecnologias usadas

* React → interface do sistema (Frontend).
* Node.js → funcionamento do sistema (Backend).
* SQLite → banco de dados (Temporário).

---

## Como funciona na prática

1. O restaurante gera um QR Code para cada mesa
2. O cliente escaneia o QR Code
3. O cardápio abre no celular
4. O cliente faz o pedido
5. O pedido vai para a cozinha
6. O restaurante acompanha tudo pelo sistema

---

## Status

Em desenvolvimento
